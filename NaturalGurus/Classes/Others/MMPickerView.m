...
@property (copy) void (^onDismissCompletionIndex)(NSInteger);
...
+(void) showPickerViewInView:(UIView *)view
    ...
    [[self sharedView] setPickerHidden:NO callBackIndex:nil];
    [self sharedView].onDismissCompletionIndex = completion;
    [view addSubview:[self sharedView]];
...
+(void)dismissWithCompletionIndex:(void (^)(NSInteger ))completion{
    [[self sharedView] setPickerHidden:YES callBackIndex:completion];
}


-(void)dismiss{
//    [MMPickerView dismissWithCompletion:self.onDismissCompletion];
    [MMPickerView dismissWithCompletionIndex:self.onDismissCompletionIndex];
}
...
-(void)setPickerHidden: (BOOL)hidden
         callBackIndex: (void(^)(NSInteger))callBack; {
...
//        selectedRow = [[_pickerViewArray objectAtIndex:0] integerValue];
    ...
    UIBarButtonItem *btnCancel = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self action:@selector(dismissWithoutCallback)];
    [btnCancel setTintColor:buttonTextColor];
    ...
    _pickerViewToolBar.items = @[btnCancel,flexibleSpace, _pickerViewBarButtonItem];
...
- (NSString *)pickerView: (UIPickerView *)pickerView
    ...
    NSDictionary *dict = [_pickerViewArray objectAtIndex:row];
    if (self.objectToStringConverter == nil){
//        return [_pickerViewArray objectAtIndex:row];
        return [dict objectForKey:@"title"];
    }
    else{
        return (self.objectToStringConverter ([dict objectForKey:@"title"]));
    }
}
...
-(void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if (self.objectToStringConverter == nil) {
//        self.onDismissCompletionIndex ((int)row);
    }
    else {
//        self.onDismissCompletionIndex ((int)row);
    }
...
- (UIView *)pickerView:(UIPickerView *)pickerView
    ...
    if (self.objectToStringConverter == nil){
        ...
        [pickerViewLabel setText:[dict objectForKey:@"title"]];
    }
    else{
//        [pickerViewLabel setText:(self.objectToStringConverter ([_pickerViewArray objectAtIndex:row]))];
        [pickerViewLabel setText:(self.objectToStringConverter ([dict objectForKey:@"title"]))];
    }
